...
//取款
int Thread::withdrawal_check()
{
	int writecnt = 0;
	int result = 0;                         
	char sql[200] = {0};                    //sql语句存放
	float account = 0;                      //存放上次余额
	...
	else
	{
		...
		{
			clientrecv.relay = WITHDRAWAL_FAIL;
		...
		else                                                                                //取钱操作
		{
			...
			clientrecv.sum_money = account-clientrecv.captial;
			if(clientrecv.sum_money < 0)
			{
				clientrecv.relay = WITHDRAWAL_FAIL;
				clientrecv.sum_money = account+clientrecv.captial;
				writecnt = send(clientfd,(char *)&clientrecv,sizeof(User),0);
			...
			else
			{
				...
				{
					clientrecv.relay = WITHDRAWAL_OK;                                           //取钱成功
					writecnt = send(clientfd,(char *)&clientrecv,sizeof(User),0);
					if(writecnt == -1)
					{
						cerr<<"send error!"<<endl;
